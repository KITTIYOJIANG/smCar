...
def execute_plan(commands: list[dict[str, Any]]) -> int:
...
def execute_command(command: dict[str, Any]) -> CommandResult:
    name = command["command"]
    if name == "move_to":
        return execute_move_to(command["target"]["row"], command["target"]["col"])
    if name == "align_to_box":
        return execute_align_to_box(command["box"]["row"], command["box"]["col"], command["direction"])
    if name == "push_box":
        return execute_push_box(command["direction"], command["cells"])
    return CommandResult("ERR_UNKNOWN", f"Unknown command {name!r}.")


def execute_move_to(row: int, col: int) -> CommandResult:
    # Replace this stub with chassis translation control.
    return CommandResult("OK", f"target row={row} col={col}")


def execute_align_to_box(row: int, col: int, direction: str) -> CommandResult:
    # Replace this stub with precise pose alignment behind the box.
    return CommandResult("OK", f"box row={row} col={col} direction={direction}")


def execute_push_box(direction: str, cells: int) -> CommandResult:
    # Replace this stub with low-speed push control and box-motion verification.
...
def main() -> int:
    ...
    parser.add_argument("plan_json", help="Path to planner JSON output.")
