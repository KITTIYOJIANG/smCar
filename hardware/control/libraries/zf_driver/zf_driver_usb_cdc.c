...
__weak void usb_cdc_user_receive_callback(const uint8 *buffer, uint32 length)
{
    (void)buffer;
    (void)length;
}
...
static uint8 usb_cdc_can_write(void)
{
    return usb_cdc_com_open_flag || ((1U == s_cdcVcom.attach) && (1U == s_cdcVcom.startTransactions));
}
...
void usb_cdc_receive_buffer_callback(void)
{
    usb_cdc_user_receive_callback(usb_cdc_receive_buff, usb_cdc_receive_size);
